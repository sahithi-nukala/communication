...
int main(int argc, const char** argv)
{
    ...
    std::cout << "Starting consumer with scenario " << static_cast<std::size_t>(test_configuration.scenario)
              << ", number of samples to receive per iteration " << score::mw::com::test::kInitialSamplesToSend.size()
              << " and number of send iterations " << num_send_iterations << std::endl;

    score::mw::com::test::RunConsumer(
        test_configuration.scenario, score::mw::com::test::kInitialSamplesToSend.size(), stop_source.get_token());
